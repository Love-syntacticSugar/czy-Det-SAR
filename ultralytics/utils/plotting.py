...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        if self.pil:  # use PIL
            ...
            self.sf = 0.5  # cv2中这个是font scale，而pil中的font_size是像素大小
        # Pose
        self.skeleton = [
            [16, 14],
            [14, 12],
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            if rotated:
                p1 = box[0]
                # SEEME 由于框的大小和字体宽度共用lw，故这里*2，希望框大一些
                self.draw.polygon([tuple(b) for b in box], width=self.lw * 2, outline=color)
            else:
                p1 = (box[0], box[1])
...
def plot_images(
    ...
    # Annotate
    fs = int((h + w) * ns * 0.005)  # font size SEEME 改了这里
    ...
    parent_dir = fname.parent  # 获取父目录
    if not parent_dir.exists():  # 检查父目录是否存在，如果不存在则创建
