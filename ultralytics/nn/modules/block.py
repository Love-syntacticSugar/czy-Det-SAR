...
class DySample(nn.Module):
    def __init__(self, in_channels, scale=2, style='lp', groups=4, dyscope=False):
        ...
        if style == 'pl':
            # 1.指数运算（**）的优先级高于取模运算（%）。这意味着，scale ** 2 将首先被计算，然后结果将用于执行取模运算
            assert in_channels >= scale ** 2 and in_channels % scale ** 2 == 0
        # 将输入通道数分为group组，这里用来判断是否能整除
...
# SEEME 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 DyHead start🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
# 注意！！mmcv的安装需要和pytorch的版本相对应，见官网：https://mmcv.readthedocs.io/zh-cn/latest/get_started/installation.html
...
from timm.models.layers import DropPath, trunc_normal_  # !pip install timm 这是一个用pytorch实现的工具包
...
class LSKblock(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv0 = nn.Conv2d(dim, dim, 5, padding=2, groups=dim)  # (k,d)-->(5,1)
        self.conv_spatial = nn.Conv2d(dim, dim, 7, stride=1, padding=9, groups=dim, dilation=3)  # (k,d)-->(7,3)
        self.conv1 = nn.Conv2d(dim, dim // 2, 1)  # 对应论文式4--分支1
        self.conv2 = nn.Conv2d(dim, dim // 2, 1)  # 对应论文式4--分支2
    ...
    def forward(self, x):
        ...
        attn = torch.cat([attn1, attn2], dim=1)  # attn1的shape为（batch_size,dim,H,W）
        avg_attn = torch.mean(attn, dim=1, keepdim=True)  # keepdim=True保持结果的shape不变仍为4维(b,1,H,W)
...
class my_DWConv(nn.Module):
    def __init__(self, dim=768):  # 768搞的像2维一样，但事实上确实比较奇怪，这里的通道数确实用的很大！！
        super(my_DWConv, self).__init__()
        self.dwconv = nn.Conv2d(dim, dim, 3, 1, 1, bias=True, groups=dim)  # 输入和输出的shape完全不变
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features  # 论文中用的是in_features
        hidden_features = hidden_features or in_features  # 论文中用的是hidden_features
        self.fc1 = nn.Conv2d(in_features, hidden_features, 1)
        self.dwconv = my_DWConv(hidden_features)
        self.act = act_layer()
        self.fc2 = nn.Conv2d(hidden_features, out_features, 1)
        self.drop = nn.Dropout(drop)  # 默认不drop（注意!dropout层的输入也可以是4维张量哦）
...
class Block(nn.Module):
    def __init__(self, dim, mlp_ratio=4., drop=0., drop_path=0., block_norm=False, norm_cfg=None, act_layer=nn.GELU):
        super().__init__()
        self.block_norm = block_norm
        self.dim = dim
        self.flag = False  # 用来标识本次input是元组类型还是数值类型，默认数值类型
        self.norm = None
    ...
    def forward(self, x):
        ...
        if self.block_norm:
            ...
            return x
        return (x, H, W) if self.flag else x
