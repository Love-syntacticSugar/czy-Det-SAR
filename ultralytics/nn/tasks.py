...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if verbose:
            self.info()
            LOGGER.info(
                "注意！以上信息中：GFLOPs是以image_size为(640*640)进行计算的！"
                "若你想计算其它image_size，请修改nn.task.py中DetectionModel类的__init__方法中的self.info()为self.info(imgsz=xxx)")
...
def attempt_load_weights(weights, use_ema_or_origin="ema", device=None, inplace=True, fuse=False, ):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        assert use_ema_or_origin in ["ema", "origin"], "use_ema_or_origin参数只允许传'ema'和'origin'"
        if use_ema_or_origin == "ema":
            model = ckpt.get("ema").to(device).float()  # FP32 model
        elif use_ema_or_origin == "origin":
            model = ckpt["model"].to(device).float()  # FP32 model
        LOGGER.info(f"已将{w}中的{use_ema_or_origin}模型权重装载！")
        assert isinstance(model, nn.Module), "读取的pt文件中没有model，请检查！"
...
def attempt_load_one_weight(weight, use_ema_or_origin="ema", device=None, inplace=True, fuse=False):
    ...
    if use_ema_or_origin == "ema":
        ...
        model = ckpt["model"].to(device).float()  # FP32 model
    LOGGER.info(f"已将{weight}中的{use_ema_or_origin}模型权重装载！")
    assert isinstance(model, nn.Module), "读取的pt文件中没有model，请检查pt文件或use_ema_or_origin参数的设置！"
